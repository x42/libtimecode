...
# - src/timecode/timecode.h (this is the MAIN VERSION used by configure) 
#   and re-run autogen.sh to pick it up.
# - Changelog 
# - debian/changelog (for the debian package)

echo "Update version number -- edit 3 files: src/timecode/timecode.h ChangeLog debian/changelog"
echo -n "launch editor ? [Y/n]"
read -n1 a
echo

if test "$a" != "n" -a "$a" != "N"; then
	${EDITOR} src/timecode/timecode.h debian/changelog ChangeLog
	sh autogen.sh
fi
...
git commit -m "finalize changelog v${VERSION}" src/timecode/timecode.h ChangeLog debian/changelog doc/man/man3/timecode.h.3
...
make dist
ls -l "libtimecode-${VERSION}.tar.gz"

GITREPO="x42/libtimecode"
